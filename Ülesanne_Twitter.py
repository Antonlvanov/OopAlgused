...
def find_fastest_growing(tweets: list) -> Tweet:
    ...
    def growth_rate(tweet):
        return tweet.rt/ tweet.time
    
    fastest_growing = tweets[0]
    for tweet in tweets[1:]:
        if growth_rate(tweet) > growth_rate(fastest_growing):
...
def sort_by_popularity(tweets: list) -> list:
    ...
    def popularity_key(tweet):
        return -tweet.retweets, tweet.time
    
    return sorted(tweets, key=popularity_key)
...
def sort_hashtags_by_popularity(tweets: list) -> list:
    ...
    for tweet in tweets:
        ...
        for word in words:
            if word.startswith('#'):
                if word not in hashtag_popularity:
                    hashtag_popularity[word] = 0
    ...
    sorted_hashtags = sorted(
        hashtag_popularity.items(),
        key=sort_key)

    hashtags = []
    for hashtag, _ in sorted_hashtags:
